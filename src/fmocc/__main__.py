...
def main():
    ...
    try:
        ...
        logger.info(f"[ENERGY INFO] {processor.config.method} Correlation Energy: {E_cc:.9f}")
        logger.info(f"[ENERGY INFO] Total {processor.config.method} Energy: {E_cc_tot:.9f}")

        fmocc_dir = os.path.dirname(__file__)
        logger.info(f"[INFO] Clearing __pycache__ in directory: {fmocc_dir}")
        HelperFunction.clear_pycache(fmocc_dir)
    
    except Exception as e:
        logger.error(f"[ERROR] Error running FMO-CC: {e}")
        sys.exit(1)
